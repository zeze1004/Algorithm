...
void union(int node1, int node2) {
    node1 = findParent(node1); // x 부모를 찾고
    node2 = findParent(node2); // y 부모를 찾고
    if (parent[node1] != parent[node2]){   // 두 노드의 부모가 다르다면
        parent[node2] = parent[node1];  // 어는 한 쪽 노드의 부모를 연결되는 다른 한쪽 노드로 설정
    }
...
int main() { 
    ...
    for ( int i = 0; i < E; i++) {
        cin >> A >>  B >>  C;
        parent.push_back(A);
        parent.push_back(B);    // 2n, 2n+1
        weight.push_back(C);    // n 
    }
    // ?ㅅ? 이걸 이제 어떻게 하지,,,
